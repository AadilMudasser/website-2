...
function HowItWorks() {
  ...
  useEffect(() => {
    const updateProgress = () => {
      ...
      const raw = (viewport - rect.top) / (viewport + rect.height * 0.65);
      const progress = Math.max(0, Math.min(1, raw));
      element.style.setProperty("--timeline-progress", `${progress * 100}%`);
    };
    updateProgress();
  ...
  return h(
    ...
    h(
      ...
      h("a", { className: "button primary", href: "#/book" }, "Get Started")
    ),
    h(
      "div",
      { className: "timeline" },
        ...
        h(
          ...
          h(
            ...
            h(
              ...
              group.subs.map((sub) => h("span", { key: sub }, sub))
            )
          )
        )
      )
...
function WeightLossPage() {
  return h(
    ...
    h(ExpandableTreatmentList, {
      eyebrow: "Weight Loss options",
      title: "Sub-services",
      items: ["Injectables", "Oral Tablets", "Lifestyle", "Maintenance Therapy"],
    }),
    h(Calculator),
    h(FinalCta)
  );
...
function WomensHealthPage() {
  return h(
    ...
    h(ExpandableTreatmentList, {
      eyebrow: "Women’s Health options",
      title: "Sub-services",
      items: ["Menopause (including HRT)", "Bone Health", "Pelvic Health"],
    }),
    h(FinalCta)
  );
}
